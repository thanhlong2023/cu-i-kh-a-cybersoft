...
export default function InfoMain(props) {
  const { projectDetail, idProject } = props
  ...
  const des = `${projectDetail.description}`
  ...
  const {
    ...
    onSubmit: (values) => {
      ...
      // console.log(payload)
      createTask(payload, idProject)
        .then((resp) => {
          alert('Tạo task thành công')
          // navigate(`/projectdetail/${idProject}`)
          location.reload()
        })
        .catch((err) => {
  ...
  return (
    <>
      ...
      <section>{ReactHtmlParser(des)}</section>
      ...
      <div className="info mb-3" style={{ display: 'flex' }}>
        ...
        <button
          ...
          style={{ marginLeft: 20, fontSize: '15px', fontWeight: '500' }}
